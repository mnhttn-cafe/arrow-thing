...
- **`MainMenuController`** — extends `NavigableScene`. Drives the main menu: Play, Continue (when save exists), Settings, Leaderboard (trophy button, top-right), and Quit (desktop only). GitHub/Discord social links at bottom-left. Scene transitions use `SceneNav.Push`. Keyboard nav graph: vertical column (Leaderboard → Play → Continue? → Settings → social row), with Quit top-left linked horizontally to Leaderboard.

### Board Size Select (`Solo Size Select` Scene)

- **`SoloSizeSelectController`** — extends `NavigableScene`. Flex-wrap preset grid (Small 10×10, Medium 20×20, Large 40×40, XLarge 100×100, Custom) with responsive layout detection via `GeometryChangedEvent` — nav graph rebuilds on resize. Custom button toggles width/height `SnapSlider` panel. Back (top-left) and Trophy/Leaderboard (top-right) buttons. Selection state (`_selectedWidth`, `_selectedHeight`, `_isCustomSelected`) survives scene stack disable/enable via `SaveState`/`RestoreState`.
...
- **`SceneNav`** — static scene stack. `Push(scene)` disables current scene (`SetActive(false)`) and loads new scene additively. `Pop()` unloads current and re-enables previous. `Replace(scene)` swaps current (Play Again, Quick Reset). Controllers save/restore state across the disable/enable cycle via `NavigableScene.SaveState`/`RestoreState`. UIDocument recreates its visual tree on re-enable, so `BuildUI` re-wires all elements fresh each time.
...
- **`NavigationCoverageTests`** — verifies every visible `Button` in each scene's UXML is reachable via keyboard navigation. Loads UXML, instantiates the scene controller, then walks all buttons checking each is present in `FocusNavigator.Active`'s item list (or an ancestor is). Buttons inside `ConfirmModal` overlays and explicitly exempt classes are skipped. Catches regressions where a button is added to UXML without being wired into the navigator.
