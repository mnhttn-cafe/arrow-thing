...
public class ArrowDirectionTests
{
    ...
    public void HeadDirection_NextBelow_IsUp() =>
        Assert.That(
            new Arrow(new Cell[] { new(5, 5), new(5, 4) }).HeadDirection,
            Is.EqualTo(Arrow.Direction.Up)
        );

    [Test]
    public void HeadDirection_NextAbove_IsDown() =>
        Assert.That(
            new Arrow(new Cell[] { new(5, 5), new(5, 6) }).HeadDirection,
            Is.EqualTo(Arrow.Direction.Down)
        );

    [Test]
    public void HeadDirection_NextRight_IsLeft() =>
        Assert.That(
            new Arrow(new Cell[] { new(5, 5), new(6, 5) }).HeadDirection,
            Is.EqualTo(Arrow.Direction.Left)
        );

    [Test]
    public void HeadDirection_NextLeft_IsRight() =>
        Assert.That(
            new Arrow(new Cell[] { new(5, 5), new(4, 5) }).HeadDirection,
            Is.EqualTo(Arrow.Direction.Right)
        );

    [Test]
    public void Constructor_NonAdjacentCells_Throws() =>
        Assert.Throws<System.ArgumentException>(() =>
            _ = new Arrow(new Cell[] { new(0, 0), new(2, 0) })
        );
